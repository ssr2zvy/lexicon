"""run.py — ONE function, ONE output: run a source's process-data
implementation and return its exit code. This is the phase's single entry
point (exec'd by process-data.sh; never bypassed). It owns ALL environment
...
IMPL_REL = os.path.join("process-data", "process_data_impl.py")
...
def run(source_name: str, argv) -> int:
    ...
    if not os.path.isfile(impl_path):
        ...
        return 2
    os.chdir(src_root)   # BEFORE import: impl module-level code relies on
    #                      the source root as cwd
    spec = importlib.util.spec_from_file_location("process_data_impl",
                                                  impl_path)
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    from process_data import main   # this file's dir is sys.path[0]
