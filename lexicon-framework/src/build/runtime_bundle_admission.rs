...
use super::{
    ExecutableSha256, HashedRuntimeArtifact, RuntimeArtifactHashError, RuntimeManifestV1,
...
pub fn admit_http_runtime_bundle(
    ...
    let manifest_text = validate_manifest_text(&manifest_path, &manifest_bytes)?;
...
fn validate_manifest_text<'a>(
    manifest_path: &Path,
    manifest_bytes: &'a [u8],
) -> Result<&'a str, RuntimeBundleAdmissionError> {
    if manifest_bytes.is_empty() {
        return Err(RuntimeBundleAdmissionError::InvalidManifestBoundary);
    }
    ...
    let text = std::str::from_utf8(without_final_newline)
        .map_err(|_| RuntimeBundleAdmissionError::InvalidManifestBoundary)?;
...
mod tests {
    ...
    use super::{
        MAX_RUNTIME_MANIFEST_BYTES, RuntimeBundleAdmissionError, admit_http_runtime_bundle,
    };
    use crate::build::{stage_verified_http_runtime_bundle, verify_http_runtime_candidate};
